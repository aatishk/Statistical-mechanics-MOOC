...
for Nid in range(len(Ns)):
	...
	for seedId in range(len(seeds)):
		...
		for run in range(10):
			...
			pylab.title("N = "+str(N)+", Best Energy = "+str(best_energy)+", seed = "+str(seed)+", run = "+str(run))
			pylab.axis('scaled')
			pylab.axis([0.0, 1.0, 0.0, 1.0])
			pylab.savefig('simulated_annealing_best_path_N%i_seed%i_run%i.png' % (N, seed, run))
			pylab.close()